...
async function loadPosts(){
    ...
    let postsHtml = postsJson.map(postInfo => {
        ...
        return `
        <div class="post">
            <p><strong>Post from ${postInfo.posterName}</strong>:</p>
...
async function postUrl(){
    ...
    let name = document.getElementById("posterNameInput").value;

    try{
        await fetchJSON(`api/${apiVersion}/posts`, {
            method: "POST",
            body: {url: url, description: description, posterName: name}
        })
    }catch(error){
